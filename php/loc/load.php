...
<?php
  ...
  $query = "SELECT * FROM locTest WHERE user_id='$user_id' ORDER BY id DESC";
...
while($row = mysqli_fetch_array($result))
  {
  $map_url = 'http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/map.php?a='.$row['latitude'].'&n='.$row['longitude'].'&r='.$row['accuracy'].'&t=HYBRID';
  $map_urli = 'http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/map.php?a='.$row['latitude'].'&n='.$row['longitude'].'&r='.$row['accuracy'].'&t=HYBRID&h='.$row['heading'].'&s='.$row['speed'];
  $msg .= "<tr>";
  $msg .= "<td style='background-color:#FFBF5F;font-weight:bold;text-align:center;'>" . $row['text'] ."<br/>". $row['geoCode']. "</td>";
  $msg .= "<td bgcolor='pink'><ul><li>latitude:".$row['latitude']."</li>";
...
//  $msg .= "<td>" . $row['accuracy'] . "</td>";
//  $msg .= "<td>" . $row['altitude'] . "</td>";
//  $msg .= "<td>" . $row['altitudeAccuracy'] . "</td>";
//  $msg .= "<td>" . $row['heading'] . "</td>";
//  $msg .= "<td>" . $row['speed'] . "</td>";
//  $msg .= "<td>" . $row['locTime'] . "</td>";
//  $msg .= "<td>" . $row['text'] . "</td>";
  $msg .= "</tr>";
  }
$msg .= "</table>";
  ...
  if ($num_pages > 1) {
      ...
      if ($cur_page >= 7) {
		...
		$end_loop = $cur_page + 3;
	    else if ($cur_page <= $num_pages && $cur_page > $num_pages - 6) {
